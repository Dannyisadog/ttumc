...
var scheduleVue = new Vue({
  ...
  methods: {
    checkSchedule: function checkSchedule(dateTime) {
      orderCheck(dateTime);
    },
    orderSchedule: function orderSchedule(identity, dateTime) {
      order(identity, dateTime);
    },
    deleteSchedule: function deleteSchedule(schedule_id) {
...
function getSchedules() {
  $.ajax({
    ...
    error: function error() {
      toastr__WEBPACK_IMPORTED_MODULE_0___default.a.error("系統發生錯誤");
    }
  });
}
...
function _deleteSchedule(schedule_id) {
  $.ajax({
    ...
    dataType: 'json',
    success: function success(resp) {
      if (resp.success) {// toastr.info(resp.msg);
      } else {
  ...
  $.ajax({
    ...
    success: function success(resp) {
      ...
      if (resp.status === true && resp.can_multi_order === true) {
        var identities = resp.identities;
...
function order(identity, datetime) {
  $.ajax({
    ...
    dataType: 'json',
    success: function success(resp) {// toastr.success("預約成功");
    },
    error: function error(xhr) {
      toastr__WEBPACK_IMPORTED_MODULE_0___default.a.error("預約失敗");
    },
    complete: function complete() {
...
function createBackground() {
  ...
  function draw() {
    ...
    for (var _i = 0, _x = stars.length; _i < _x; _i++) {
      var starI = stars[_i];
      ctx.moveTo(starI.x, starI.y);
      if (distance(mouse, starI) < 150) ctx.lineTo(mouse.x, mouse.y);

      for (var j = 0, _x2 = stars.length; j < _x2; j++) {
        var starII = stars[j];
...
module.exports = __webpack_require__(/*! /Users/danny/www/ttumc/resources/js/schedule.js */"./resources/js/schedule.js");
